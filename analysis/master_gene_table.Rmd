...
# Annotate Lanner
marker_genes <- read.table("./data/other/Lanner/Fig1C.temp.lineagemarker.genes.highlight.csv", sep = "\t", header = T)
colnames(marker_genes) <- c("Lanner_germlayer", "name", "anno")
marker_genes$anno <- NULL

master_df <- left_join(master_df, marker_genes, by = "name")
master_df[is.na(master_df$Lanner_germlayer), "Lanner_germlayer"] <- "None"
```
...
first_cols <- c("name", "seqnames", "start", "end", "strand", "k27_bivalency_grp", "court_bivalent", "Lanner_germlayer")
order <-
  c(first_cols,
...
filename <- paste0(filename, "_annotated.tsv")
